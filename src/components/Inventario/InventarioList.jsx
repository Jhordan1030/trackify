import { Package, AlertTriangle, TrendingUp, Archive, Plus, Edit, Upload, Trash2, Box, ShieldAlert, Power, PowerOff } from 'lucide-react';
            ...
            <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-3 sm:gap-4">
                <div className="flex-1 min-w-0">
                    <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900">
                        ...
                        <span className="text-blue-600"> ({inventario.length})</span>
                    </h2>
            ...
            <div className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-6 border border-gray-200">
                <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between gap-2 sm:gap-3 md:gap-4 text-xs sm:text-sm">
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 md:gap-6">
                        <span className="text-gray-600">Total: <strong className="text-gray-900">{inventario.length}</strong> productos</span>
                        {filtroActivo && (
                            <>
                                <span className="text-orange-600">
                                    Stock bajo: <strong>{inventario.filter(item => calcularStockDisponible(item) <= (item.stock_minimo || 0)).length}</strong>
                                </span>
                                <span className="text-red-600">
                                    Sin stock: <strong>{inventario.filter(item => calcularStockDisponible(item) === 0).length}</strong>
                                </span>
                            </>
                        )}
                    </div>
