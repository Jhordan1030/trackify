// src/components/Inventario/EditarProductoModal.jsx
import React, { useState, useEffect } from 'react';
import { X, Plus, Minus, ChevronDown, Save } from 'lucide-react';
...
const EditarProductoModal = ({ 
  ...
  useEffect(() => {
    if (isOpen && producto) {
      console.log('📝 Cargando datos del producto para editar:', producto);
      ...
      if (producto.variacion && Object.keys(producto.variacion).length > 0) {
        const variacionesArray = Object.entries(producto.variacion).map(([tipo, valores]) => ({
          tipo: tipo.charAt(0).toUpperCase() + tipo.slice(1), // Capitalizar
          valores: Array.isArray(valores) ? valores : [valores]
        }));
        setVariaciones(variacionesArray);
        setMostrarVariaciones(true);
      } else {
        setVariaciones([]);
        
        // Determinar si mostrar variaciones basado en la categoría
        const categoriaSeleccionada = categorias.find(cat => cat.nombre === producto.categoria);
  ...
  const handleSubmit = async (e) => {
    ...
    const productoData = {
      productoId: producto.producto_id || producto.id,
    ...
    console.log('🔍 VALIDACIÓN FINAL - Datos a actualizar:', {
      ...productoData,
    ...
    try {
      await onEditarProducto(productoData);
    } catch (error) {
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              ID: {producto.id} | SKU: {producto.sku_codigo}
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  disabled
                  title="El stock actual no se puede modificar aquí. Use ajustes de stock."
                />
                <p className="text-xs text-gray-500 mt-1">
                  Use "Ajustar Stock" para modificar el inventario
                </p>
              </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-blue-700 text-xs mt-1">
                    Los cambios se aplicarán a todos los SKUs asociados a este producto.
                    {mostrarVariaciones && variaciones.length > 0 && (
                      <span className="block mt-1 font-medium">
                        Variaciones actuales: {variaciones.map(v => v.tipo).join(' + ')}
                      </span>
                    )}
                  </p>
                </div>
